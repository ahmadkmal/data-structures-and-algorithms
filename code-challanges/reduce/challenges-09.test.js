...
const reversedString = (str) => {
  // Solution code here...
  str = str.split('');
  return str.reduce((acc,val)=>val+acc,'');
};
...
const calculateAverage = (arr) => {
  ...
  return arr.reduce((acc,val)=>{
    acc.count +=1;
    acc.sum += val;
    acc.av=acc.sum/acc.count;
    return acc;
  },{ count: 0, sum: 0,av:0 }).av;
...
const countPrimeNumbers = (arr) => {
  // Solution code here...
  return arr.reduce((acc,val)=>{return isPrime(val)?acc+1:acc;},0);
};
...
const extractStat = (statName, arr )=>{
  ...
  return arr.reduce((acc,val,index)=>{
    ...
    {
      console.log(index,val.stat.name,statName);
      acc = val;
      console.log(acc);
      return acc;
    }
...
const extractChildren = (arr) => {
  // Solution code here...
  let out = arr.filter(el=>el.name.includes('a'));
  console.log(out);
  out = out.reduce((acc,val)=>{
    console.log(acc,val.children);
    if(val.children){
    //   val.children.foreach(ele=>{acc.push(ele);});
      for (let index = 0; index < val.children.length; index++) {
        acc.push(val.children[index]);
      }
    }
    console.log(acc);
    return acc;
