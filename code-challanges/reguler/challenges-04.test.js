...
const isCapitalized = (str) => {
  // Solution code here...
  let regex= /([A-Z]\w+)/g;
  console.log('entered data',str);

  console.log('passd',str.match(regex) );
...
const citiesAtoJ = (arr) => {
  ...
  arr.forEach(element => {
    if(element.match(regex)&&!element.match(/\s/g)){
      newarr.push(element.match(regex)[0]);
    }
  });
  return newarr;
};
...
const matchMonth = (input) => {
  // Solution code here...
  var regex =/^(October|)\b/g;
  console.log('ch4',input);
...
let hangman = (str) => {
  ...
  // .replace(/([e])/ig,'_').replace(/([i])/ig,'_').replace(/([u])/ig,'_').replace(/([o])/ig,'_');
  // .replace(/([A])/g,'_').replace(/([E])/g,'_').replace(/([O])/g,'_').replace(/([U])/g,'_').replace(/([I])/g,'_');
};
