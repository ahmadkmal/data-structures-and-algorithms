  ...
  arr.forEach(element => {
    if(element.name === character){
      ...
      if(element.children.length>0){
        console.log(element.name,element.children.length,true);
        out = true;
      }else{
        out = false;
      }
  ...
  arr.forEach(element => {
    if(element.name === character){
      for (const key in element) {
        if (key === 'children' && element.children.length)
          out = true;
      }
    }
  });
...
const houseSurvivors = (arr) => {
  ...
  function num(obj){
    let out = 1;
    if(obj.spouse && !deceasedSpouses.includes(obj.spouse))
      out++;
    if(obj.children)
      out+=obj.children.length;
    return out;
