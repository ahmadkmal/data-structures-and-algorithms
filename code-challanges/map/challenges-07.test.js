...
const mapTwoToThe = (arr) => {
  // Solution code here...
  var out = arr.map((element)=>Math.pow(2, element));
  return out;
};
...
const charCode = (arr) => {
  // Solution code here...
  var out = arr.map((ele)=>ele.charCodeAt(0));
  return out;
};
...
const extractStats = (arr) => {
  // Solution code here...
  return arr.map((ele)=>{var obj ={name : ele.stat.name,total:ele.effort+ele.baseStat};
    return obj;
  });
};
