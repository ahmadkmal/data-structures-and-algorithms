...
//   console.log('stop');
  return input.reduce((acc,val)=>{
    // console.log('outer',acc,val);
    return (acc + val.reduce((acc2,val2)=>{
    //   console.log('inter',acc2,val2);
      if(val2 === target){
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(val=>{
    var divAble=(val.filter(ele=>typeof(ele)==='number'&&ele%5===0));
    return divAble.map(ele=>Math.pow(2,ele));
  });
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(ele=>ele.gender==='male'||ele.gender==='female').map(ele=>ele.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((shortest,ele)=>{
    console.log(shortest.height>ele.height);
    if(Number(ele.height) < Number(shortest.height)){
      console.log(ele.height);
      return ele;
    }
    console.log('didny change');
    return shortest;
  },data[0]).name;
};
